...
class CustomerPortal(payment_portal.PaymentPortal):
    ...
    def _prepare_quotations_domain(self, partner):
        return [("message_partner_ids", "child_of", [partner.commercial_partner_id.id]), ("state", "=", "sent")]
    ...
    def portal_order_page(self, order_id, report_type=None, access_token=None, message=False, download=False, downpayment=None, **kw):
        try:
        ...
        backend_url = (
            f"/web#model={order_sudo._name}" f"&id={order_sudo.id}" f"&action={order_sudo._get_portal_return_action().id}" f"&view_type=form"
        )
        values = {
        ...
        if order_sudo._has_to_be_paid():
            values.update(
                self._get_payment_values(
                    order_sudo, downpayment=downpayment == "true" if downpayment is not None else order_sudo.prepayment_percent < 1.0
                )
            )
    ...
    def _get_payment_values(self, order_sudo, downpayment=False, **kwargs):
        ...
        tokens_sudo = (
            request.env["payment.token"].sudo()._get_available_tokens(providers_sudo.ids, partner_sudo.id, **kwargs)
        )  # In sudo mode to read the partner's tokens (if logged out) and provider fields.

        # Make sure that the partner's company matches the invoice's company.
        company_mismatch = not payment_portal.PaymentPortal._can_partner_pay_in_company(partner_sudo, company)
        ...
        payment_form_values = {
            "show_tokenize_input_mapping": PaymentPortal._compute_show_tokenize_input_mapping(providers_sudo, sale_order_id=order_sudo.id),
        }
        payment_context = {
            "amount": amount,
            "currency": currency,
...
class PaymentPortal(payment_portal.PaymentPortal):
    ...
    def payment_pay(self, *args, amount=None, sale_order_id=None, access_token=None, **kwargs):
        ...
        if sale_order_id:
            ...
            if not payment_utils.check_access_token(access_token, order_sudo.partner_id.id, amount, order_sudo.currency_id.id):
                raise ValidationError(_("The provided parameters are invalid."))
    ...
    def _get_extra_payment_form_values(self, sale_order_id=None, access_token=None, **kwargs):
        ...
        :rtype: dict
        """
        form_values = super()._get_extra_payment_form_values(sale_order_id=sale_order_id, access_token=access_token, **kwargs)
        if sale_order_id:
            sale_order_id = self._cast_as_int(sale_order_id)
